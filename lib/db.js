...
/**
 * Get scores sorted by score descending, optionally filtered by grade.
 */
export async function getScores(limit = 10, grade) {
  try {
    const scores = await prisma.score.findMany({
      where: typeof grade === 'number' ? { grade } : undefined,
      orderBy: {
        score: 'desc',
      },
      take: limit,
 ...
 */
export async function createScore(playerName, scoreValue, gradeValue = 0) {
  // Validation
  ...
  if (typeof gradeValue !== 'number' || gradeValue < 0 || !Number.isInteger(gradeValue)) {
    throw new Error('Grade must be a non-negative integer');
  }
